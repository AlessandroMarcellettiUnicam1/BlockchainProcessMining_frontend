...
import {
  setupBlocklyEditor,
  generateRuleStringFromWorkspace,
} from "../coblockly.ts";
...
export default function CoBlocklyEditor({
  onRuleTranslated,
}: CoBlocklyEditorProps) {
  ...
  useEffect(() => {
    if (blocklyDivRef.current && !workspaceRef.current) {
      workspaceRef.current = setupBlocklyEditor(
        blocklyDivRef.current,
        isDarkMode,
      );
    } else if (workspaceRef.current) {
      workspaceRef.current.setTheme(
        isDarkMode ? DarkTheme : Blockly.Themes.Classic,
      );
    }
  }, [isDarkMode]);
      ...
      <div className="col-12">
        <div className="card">
          ...
          <div className="card-footer text-muted small">
            Drag and drop blocks to model your rule, then click "Translate Rule"
            to generate the textual rule
          </div>
        </div>
      </div>
    </div>
